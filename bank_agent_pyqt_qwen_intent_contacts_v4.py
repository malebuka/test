# bank_agent_pyqt_qwen_intent_contacts_v4.py
# Полный пример одним файлом:
# PyQt + Ollama + основной агент + Qwen intent-классификатор + FSM step-логика.
#
# Что есть:
# 1. До подтверждения личности финансовые данные не раскрываются.
# 2. Qwen3-4B-Instruct через Ollama используется только как intent-классификатор.
# 3. Основной агент t-8b-base формулирует реплики.
# 4. Причина неоплаты спрашивается отдельным шагом.
# 5. Варианты урегулирования:
#    - помощь родственников / друзей, занять, перекредитоваться;
#    - частичная оплата;
#    - реструктуризация;
#    - передача автомобиля.
# 6. Если клиента удалось убедить на один из вариантов — обязательно спрашивается,
#    изменилась ли контактная информация.
# 7. Если клиент отказался от всех вариантов — контактная информация всё равно спрашивается.
# 8. Если клиент говорит "да, контакты изменились", бот НЕ завершает диалог,
#    а переходит на step 15 и просит назвать актуальный номер/email/время связи.
# 9. Итог договорённости или итог без договорённости собирается кодом, а не моделью.
#
# Установка:
...
#
# Запуск:
#   python bank_agent_pyqt_qwen_intent_contacts_v4.py
...
# Модель intent-классификатора.
INTENT_MODEL = "qwen3:4b-instruct"

# Показывать серую строку с intent в окне.
SHOW_DEBUG = True
...
# Эти данные нельзя передавать модели до подтверждения личности.
PRIVATE_DATA = {
...
class DialogState:
    ...
    # Лестница урегулирования:
    # 0 — ещё не предлагали варианты;
    ...
    # Данные для итогового подытоживания.
...
def contains_contact_details(text: str) -> bool:
    """
    Простая страховка: если клиент сказал "да, изменился, новый номер 8999...",
    Qwen иногда может вернуть contact_info_changed, а не contact_info_provided.
    Тогда код сам понимает, что контакт уже предоставлен.
    """
    t = normalize(text)
    ...
    # Телефон: хотя бы 7 цифр в тексте.
    digits = re.sub(r"\D", "", t)
    if len(digits) >= 7:
        return True
    ...
    # Удобное время связи.
    time_markers = [
        "звоните", "перезвоните", "после", "до ", "с ", "вечером",
        "утром", "днем", "днём", "завтра", "сегодня", "в обед",
    ]
    if any(marker in t for marker in time_markers):
        return True
...
def safe_json_loads_from_text(text: str) -> Dict[str, Any]:
    """
    Qwen иногда возвращает не чистый JSON, а markdown или текст + JSON.
...
# ============================================================
# 5. РАЗРЕШЁННЫЕ INTENT ПО ШАГАМ
# ============================================================

def allowed_intents_for_state(state: DialogState) -> List[str]:
    """
    Qwen видит только intent, разрешённые для текущего шага.
    Это защита от перескоков.
    Например, на step 6 нельзя вернуть agrees_restructuring.
    """
    common = [
        "asks_any_question",
        "rude_or_offtopic",
    ...
    # step 12: готовность подписать документы и передать авто.
    if state.step == 12:
        return [
    ...
    # step 13: последствия / возможный суд.
    if state.step == 13:
        return [
            "agrees_sources_help",
            "agrees_partial_payment",
    ...
    # step 14: спрашиваем, изменилась ли контактная информация.
    if state.step == 14:
        return [
    ...
    # step 15: клиент сказал, что контакты изменились,
    # теперь ждём именно новые контактные данные.
    if state.step == 15:
        return [
            "contact_info_provided",
            "contact_info_same",
...
def current_question_for_step(state: DialogState) -> str:
    questions = {
        ...
        15: "Подскажите, пожалуйста, актуальный номер телефона, email или удобное время для связи.",
    }
...
# ============================================================
# 6. PROMPT ДЛЯ QWEN INTENT-МОДЕЛИ
# ============================================================
...
Контакты:
- contact_info_changed: клиент говорит только то, что контактные данные изменились, но НЕ назвал сами новые данные. Примеры: "да, изменились", "номер поменялся", "контакты другие".
- contact_info_same: клиент говорит, что контактные данные не изменились / те же / актуальны.
- contact_info_provided: клиент дал новый телефон, email, адрес или удобное время связи. Примеры: "новый номер 89991234567", "пишите на mail@example.com", "звоните после 18".
- declines_contact_info: клиент отказался говорить по контактам.

Важное правило по контактам:
- Если клиент сказал "да, изменились" без номера/email/времени связи, это contact_info_changed.
- Если клиент сказал "да, новый номер 89991234567", это contact_info_provided.
- Если клиент сказал "нет, не изменились", это contact_info_same.
...
def choose_goal(intent_result: Dict[str, Any], state: DialogState, user_text: str) -> str:
    ...
    # Если Qwen не уверен, не двигаем шаг.
    ...
    # После подтверждения личности:
    # вопросы, грубость, оффтопик не должны двигать сценарий.
    if intent in ["asks_any_question", "rude_or_offtopic", "free_client_message", "unclear"]:
        return "answer_or_react_keep_step"
    ...
    # ========================================================
    # STEP 6: РОДСТВЕННИКИ / ЗАНЯТЬ / ПЕРЕКРЕДИТОВАТЬСЯ
    # ========================================================
    ...
    # ========================================================
    # STEP 12: ГОТОВНОСТЬ ПЕРЕДАТЬ АВТО В 3 ДНЯ
    # ========================================================
    ...
    # ========================================================
    # STEP 14: СПРОСИЛИ, ИЗМЕНИЛАСЬ ЛИ КОНТАКТНАЯ ИНФОРМАЦИЯ
    # ========================================================

    if state.step == 14:
        # ВАЖНО:
        # contact_info_changed теперь НЕ завершает диалог.
        # Он переводит на step 15, где бот попросит сами новые данные.
        if intent == "contact_info_changed":
            # Страховка: если в этой же реплике уже есть номер/email/время связи,
            # считаем, что контакт предоставлен.
            if contains_contact_details(user_text):
                ...
                if state.selected_solution:
                    ...
                    return "summarize_agreement_and_end"

                state.state = "NO_AGREEMENT_END"
                return "summarize_no_agreement_and_end"

            state.step = 15
            return "ask_new_contact_details"

        if intent == "contact_info_provided":
            state.contact_update = str(extracted).strip()
            state.ended = True
        ...
        if intent == "declines_contact_info":
    ...
    # ========================================================
    # STEP 15: КОНТАКТЫ ИЗМЕНИЛИСЬ, ЖДЁМ САМИ ДАННЫЕ
    # ========================================================

    if state.step == 15:
        if intent == "contact_info_provided" or contains_contact_details(user_text):
            state.contact_update = str(extracted).strip()
            state.ended = True
        ...
        if intent == "contact_info_same":
            state.contact_update = "клиент уточнил, что контактная информация всё же не изменилась"
            state.ended = True
        ...
        if intent == "declines_contact_info":
            state.contact_update = "клиент отказался назвать актуальные контактные данные"
            state.ended = True
        ...
        # Если снова сказал "да, изменились", но всё ещё не дал данные.
        if intent == "contact_info_changed":
            return "ask_new_contact_details"
...
def refusal_goal(state: DialogState) -> str:
    ...
    После полного отказа НЕ завершаем сразу:
    всё равно переводим на step 14 и спрашиваем контактную информацию.
    """
    ...
    # Но перед завершением всё равно спрашиваем контакты.
    state.state = "NO_AGREEMENT_CONTACT_CHECK"
    state.step = 14
    state.selected_solution = None
    state.selected_solution_label = None
    return "ask_contact_change_no_agreement"


# ============================================================
# 8. PROMPT ДЛЯ ОСНОВНОЙ МОДЕЛИ
# ============================================================
...
def goal_instruction(goal: str) -> str:
    instructions = {
        ...
        "ask_contact_change_after_agreement": (
            ...
            "попроси сказать, что именно изменилось. Не подводи итог, пока клиент не ответит по контактам."
        ),
        "ask_contact_change_no_agreement": (
            "Кратко зафиксируй, что на данный момент договоренность по оплате не достигнута. "
            "После этого обязательно спроси, изменилась ли контактная информация клиента. "
            "Если изменилась, попроси сказать, что именно изменилось. Не завершай разговор, "
            "пока клиент не ответит по контактам."
        ),
        "ask_contacts_again": (
            "Повтори вопрос: изменилась ли контактная информация клиента. Если изменилась, "
            "попроси назвать актуальные данные. Не завершай разговор."
        ),
        "ask_new_contact_details": (
            "Клиент сказал, что контактная информация изменилась, но ещё не назвал новые данные. "
            "Попроси назвать актуальный номер телефона, адрес электронной почты или удобное время связи. "
...
def build_agreement_summary_message(state: DialogState) -> str:
...
def build_no_agreement_summary_message(state: DialogState) -> str:
    reason = state.debt_reason or "причина неоплаты не указана"
    contacts = state.contact_update or "информация по контактам не уточнена"
...
""".strip()
        ...
        # Страховка для причины неоплаты:
        # на step 4 почти любой содержательный ответ считается причиной, если это не вопрос.
        ...
        # Страховка для контактов:
        # если на step 14/15 в тексте есть телефон/email/время связи,
        # считаем это contact_info_provided.
        if self.dialog_state.step in [14, 15] and contains_contact_details(user_input):
            if data["intent"] in ["contact_info_changed", "free_client_message", "unclear"]:
                data = {
                    "intent": "contact_info_provided",
                    "confidence": 0.85,
                    "is_answer_to_current_question": True,
                    "extracted_value": user_input,
                    "short_reason": "В сообщении найдены контактные данные",
                }
    ...
    def send_message(self):
        ...
        try:
            ...
            # Если клиент задал вопрос / сказал не по теме / грубит —
            # не затираем предыдущую цель.
            ...
            # 4. Итоги собираем кодом.
            if goal == "summarize_agreement_and_end":
                bot_text = build_agreement_summary_message(self.dialog_state)

            elif goal == "summarize_no_agreement_and_end":
                bot_text = build_no_agreement_summary_message(self.dialog_state)
            ...
            # 6. Если диалог завершён, блокируем ввод.
    ...
    def render_debug(self, intent_result: Dict[str, Any]):
        ...
        debug_html = (
            ...
            f'is_answer: {is_answer}, extracted: {extracted}, reason: {reason}'
            f'</span><br>'
        )
