...
# Это НЕ stage. Это запасной список против зацикливания.
# Основная модель всё равно сама ведёт SETTLEMENT по prompt и истории.
# Этот список используется только если модель повторила ответ или дала пустую общую фразу
# вроде "давайте выберем другой вариант", но не предложила конкретный вариант.
SETTLEMENT_FALLBACK_OPTIONS = [
    {
        "key": "помощь родственников или друзей",
        "reply": (
            "Тогда предлагаю рассмотреть помощь родственников, друзей или знакомых: "
            "сможете обратиться к кому-то, чтобы закрыть просрочку в ближайшее время?"
        ),
    },
    {
        "key": "занять средства",
        "reply": (
            "Можно также временно занять средства, чтобы не допустить дальнейшего увеличения задолженности. "
            "Сможете рассмотреть такой вариант?"
        ),
    },
    {
        "key": "перекредитование или рефинансирование",
        "reply": (
            "Если сейчас нет нужной суммы, можно рассмотреть перекредитование или рефинансирование "
            "в другой организации. Готовы рассмотреть такой вариант?"
        ),
    },
    {
        "key": "частичная оплата",
        "reply": (
            "Если всю сумму внести не получается, можно зафиксировать хотя бы частичную оплату. "
            "Какую сумму вы сможете внести и когда?"
        ),
    },
    {
        "key": "реструктуризация",
        "reply": (
            "Тогда можно рассмотреть реструктуризацию: продление срока кредита для снижения ежемесячного платежа, "
            "ставка от 18,9% годовых. Готовы рассмотреть этот вариант?"
        ),
    },
    {
        "key": "передача автомобиля",
        "reply": (
            "Также можно рассмотреть передачу автомобиля для дальнейшего урегулирования задолженности банком. "
            "Готовы обсудить этот вариант?"
        ),
    },
    {
        "key": "последствия отсутствия договоренности",
        "reply": (
            "Если договориться не получится, задолженность может увеличиваться из-за начислений, "
            "а кредитная история может ухудшиться. Давайте всё же выберем реальный вариант урегулирования."
        ),
    },
]
...
def option_was_seen(option_key: str, state: DialogState) -> bool:
    """
    Проверяем не точным равенством, а по вхождению слов, чтобы:
    "реструктуризация" и "реструктуризация кредита" считались одним вариантом.
    """
    key = option_key.lower().replace("ё", "е")

    memory_items = state.offered_options + state.rejected_options
    memory = " | ".join(memory_items).lower().replace("ё", "е")

    if not memory:
        return False

    # Для коротких ключей используем прямую проверку.
    if key in memory:
        return True

    # Мягкие синонимы.
    synonyms = {
        "помощь родственников или друзей": ["родствен", "друз", "знаком"],
        "занять средства": ["занять", "займ", "в долг"],
        "перекредитование или рефинансирование": ["перекредит", "рефинанс"],
        "частичная оплата": ["частич", "часть"],
        "реструктуризация": ["реструктур"],
        "передача автомобиля": ["авто", "автомоб", "машин", "передача автомобиля"],
        "последствия отсутствия договоренности": ["последств", "кредитная история", "начислен"],
    }

    for token in synonyms.get(option_key, []):
        if token in memory:
            return True
...
def choose_next_fallback_option(state: DialogState) -> Optional[dict]:
    """
    Это не stage и не основная логика сценария.
    Это страховка: если модель зациклилась или дала общую фразу,
    выбираем первый вариант, который ещё не был предложен/отклонён.
    """
    for option in SETTLEMENT_FALLBACK_OPTIONS:
        if not option_was_seen(option["key"], state):
            return option

    return None


def looks_like_empty_settlement_reply(bot_text: str) -> bool:
    """
    Фразы, которые звучат как "выберем что-нибудь", но не содержат конкретного варианта.
    Именно они вызывали зацикливание.
    """
    text = bot_text.lower().replace("ё", "е")

    empty_phrases = [
        "давайте выберем другой возможный вариант",
        "выберем другой вариант",
    ...
    has_empty_phrase = any(p in text for p in empty_phrases)

    concrete_words = [
        "родствен", "друз", "знаком", "занять", "в долг",
        "перекредит", "рефинанс", "частич", "реструктур",
        "авто", "автомоб", "машин", "суд", "кредитная история",
        "контакт", "телефон", "номер",
    ]

    has_concrete_option = any(w in text for w in concrete_words)

    return has_empty_phrase and not has_concrete_option


def build_forced_settlement_reply(state: DialogState) -> str:
    """
    Возвращает конкретный вариант вместо общей фразы.
    Параллельно обновляет память, чтобы этот fallback тоже не повторялся.
    """
    option = choose_next_fallback_option(state)

    if option is None:
        add_unique(state.offered_options, "последствия отсутствия договоренности")
        return (
            "Если договориться не получится, банк может продолжить работу по договору "
            "в установленном порядке. Давайте всё же выберем реальный вариант урегулирования."
        )

    add_unique(state.offered_options, option["key"])
    return option["reply"]
...
- Если выбран блок SETTLEMENT, обязательно предложи конкретный вариант урегулирования, а не общую фразу.
- Нельзя отвечать только фразой "давайте выберем другой вариант" без названия конкретного варианта.
...
""".strip()
    ...
    def send_message(self):
        ...
        try:
            ...
            # Защита от повтора и пустой общей фразы.
            # Раньше здесь была статичная фраза:
            # "Поняла вас. Давайте выберем другой возможный вариант..."
            # Из-за нее бот мог застревать, потому что конкретный вариант не предлагался.
            repeated = (
                self.dialog_state.last_bot_text
                and bot_text.strip() == self.dialog_state.last_bot_text.strip()
            )

            empty_settlement = (
                self.dialog_state.block == "SETTLEMENT"
                and looks_like_empty_settlement_reply(bot_text)
            )

            if repeated or empty_settlement:
                if self.dialog_state.block == "SETTLEMENT":
                    bot_text = build_forced_settlement_reply(self.dialog_state)
                else:
                    bot_text = "Уточните, пожалуйста, ваш ответ."
