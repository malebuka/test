# bank_agent_pyqt_full_v2.py
# PyQt + Ollama агент для сценария взыскания.
#
# Что важно:
# - Модель вызывается 1 раз на ход.
# - State/step контролируются кодом.
# - До подтверждения личности модель НЕ видит PRIVATE_DATA.
# - Step двигается только если ответ клиента подходит текущему вопросу.
# - Вопросы клиента / грубость / не по теме НЕ двигают step.
# - Все варианты урегулирования разнесены:
...
# - Если клиент соглашается на любой вариант — агент фиксирует договоренность
#   и НЕ предлагает следующие варианты.
#
# Перед запуском:
...
def answer_matches_current_step(user_text: str, state: DialogState) -> bool:
    ...
    if state.step == 6:
        return has_any(text, [
            "да", "попробую", "подумаю", "спрошу", "попрошу", "обращусь",
            "займу", "возьму в долг", "родствен", "друз", "перекредит",
            "рефинанс", "нет", "не получится", "не вариант", "не могу",
            "не хочу", "не у кого", "никто не поможет"
        ])
    ...
    if state.step == 7:
        return (
            has_any(text, [
                "да", "смогу", "готов", "готова", "частично", "часть",
                "могу внести", "могу оплатить", "5000", "5 тысяч",
                "нет", "не могу", "не получится", "не подходит", "не вариант"
            ])
    ...
    if state.step == 8:
        return has_any(text, [
            "да", "согласен", "согласна", "готов", "готова", "реструктур",
            "подходит", "оформить", "давайте", "нет", "не хочу",
            "не подходит", "не вариант"
        ])

    # step 9: передача автомобиля.
    if state.step == 9:
        return has_any(text, [
            "да", "готов", "готова", "согласен", "согласна", "рассмотрю",
            "авто", "машин", "автомоб", "передам", "отдам", "сдам",
            "нет", "не хочу", "не готов", "не получится", "не вариант"
        ])
...
def detect_intent(user_text: str, state: DialogState) -> str:
    ...
    # -------------------------
    # Step-specific agreements.
    # Эти блоки нужны, чтобы при согласии на конкретный вариант
    # агент НЕ шел к следующему варианту.
    # -------------------------

    # step 5: оплатить в течение трех дней?
    if state.identity_confirmed and state.step == 5:
        if has_any(text, [
            "да", "смогу", "получится", "оплачу", "заплачу", "внесу",
            "сегодня", "завтра", "в течение трех дней"
        ]):
            return "payment_3_days_yes"

        if has_any(text, [
            "нет", "не смогу", "не получится", "не могу", "денег нет",
            "нет денег", "позже", "потом", "не сейчас", "платить нечем"
        ]):
            return "payment_3_days_no"

    # step 6: помощь родственников / занять / перекредитоваться.
    if state.identity_confirmed and state.step == 6:
        if has_any(text, [
            "да", "попрошу", "спрошу", "обращусь", "попробую", "подумаю",
            "займу", "возьму в долг", "у родственников", "у друзей",
            "родствен", "друз", "перекредит", "рефинанс"
        ]):
            return "agrees_sources_help"

        if has_any(text, [
            "нет", "не буду", "не получится", "не могу", "не вариант",
            "не хочу", "не у кого", "никто не поможет"
        ]):
            return "declines_sources_help"

    # step 7: частичная оплата.
    if state.identity_confirmed and state.step == 7:
        if (
            has_any(text, [
                "да", "смогу", "готов", "готова", "частично", "часть",
                "могу внести", "могу оплатить", "внесу часть", "оплачу часть",
        ...
        if has_any(text, [
            "нет", "не смогу", "не могу", "не получится", "не подходит",
            "не вариант", "не хочу"
        ]):
            return "declines_partial_payment"

    # step 8: реструктуризация.
    if state.identity_confirmed and state.step == 8:
        if has_any(text, [
            "да", "согласен", "согласна", "готов", "готова", "давайте",
            "подходит", "оформить", "реструктур", "рассмотрю"
        ]):
            return "agrees_restructuring"

        if has_any(text, [
            "нет", "не хочу", "не подходит", "не вариант", "не буду",
            "не согласен", "не согласна"
        ]):
            return "declines_restructuring"

    # step 9: передача автомобиля.
    if state.identity_confirmed and state.step == 9:
        if has_any(text, [
            "да", "готов", "готова", "согласен", "согласна", "рассмотрю",
            "передам", "отдам", "сдам", "авто", "машину", "автомобиль"
        ]):
            return "agrees_vehicle_transfer"

        if has_any(text, [
            "нет", "не хочу", "не готов", "не готова", "не вариант",
            "не получится", "машину не отдам", "авто не отдам"
        ]):
            return "declines_vehicle_transfer"

    # step 12: готов передать авто за 3 дня?
    if state.identity_confirmed and state.step == 12:
        if has_any(text, ["да", "готов", "готова", "смогу", "получится", "согласен", "согласна"]):
            return "vehicle_transfer_ready"

        if has_any(text, ["нет", "не готов", "не готова", "не смогу", "не получится", "не хочу"]):
            return "vehicle_transfer_not_ready"

    # -------------------------
    # Global explicit agreements.
    # -------------------------
...
def choose_goal(intent: str, state: DialogState) -> str:
    ...
    if state.identity_confirmed:
        # Ответ на вопрос: оплата в 3 дня.
        if intent == "payment_3_days_yes":
            state.ended = True
            state.state = "RESOLVED"
            return "confirm_full_payment_end"

        if intent == "payment_3_days_no":
            return refusal_goal(state)

        # Согласие на поиск источников средств.
        if intent == "agrees_sources_help":
            state.ended = True
            state.state = "RESOLVED"
            return "confirm_sources_help_end"
...
def decline_current_option(state: DialogState, minimum_stage: int) -> str:
    """
    Если клиент отказался от конкретного варианта, переходим к следующему.
    minimum_stage нужен, чтобы не откатиться назад.
    """
...
def refusal_goal(state: DialogState) -> str:
    ...
    Все варианты идут по отдельности.
    """
...
def advance_after_step_answer(state: DialogState) -> str:
    ...
    if state.step == 6:
        # Если согласился, detect_intent уже должен был вернуть agrees_sources_help.
        # Если дошли сюда, считаем ответ уклончивым и идем к следующему варианту.
        return decline_current_option(state, minimum_stage=1)
    ...
    if state.step == 12:
        # Если готов — detect_intent вернет vehicle_transfer_ready.
        # Если сюда попали, считаем уклончивым ответом и идем к последствиям.
        return decline_current_option(state, minimum_stage=4)
...
def goal_instruction(goal: str) -> str:
    instructions = {
        ...
        "confirm_sources_help_end": (
            "Зафиксируй, что клиент готов обратиться за помощью к родственникам или друзьям, "
            "занять средства либо рассмотреть перекредитование для погашения задолженности. "
            "Кратко уточни ближайший срок оплаты или дальнейшего контакта и попрощайся."
        ),
